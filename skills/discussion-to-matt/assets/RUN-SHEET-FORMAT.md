...
> Roadmap line NN · **UI** or **no UI**. Open a fresh session at the project root, run `/grill-with-docs`, paste the briefing block, then work the commands in order.

A sheet has four parts and no others: the two agent-facing blocks below, the briefing block the user pastes, and the commands. Only one rule binds what may live outside the briefing block — **no second copy of the intent, the scope or the anchors**. That is the drift the block exists to prevent, and it is the only thing that can drift, because everything else here is an instruction rather than a restatement.

A sheet is **stateless**: it records no depth field and no progress. `ROADMAP.md` holds both.

## Before grilling

Agent-facing. The briefing block's first line points here, which is how these instructions reach a session that only ever sees the paste. Run this before the first question, and do it silently — the user is not asked to confirm document maintenance.
...
Everything the grilling session needs, in one block. The user copies it whole — one selection, one paste, nothing to assemble.
...
````text
> Run sheet: `docs/foundation/run-sheets/NN-<slug>.md` — open it and follow its *Before grilling* block before your first question.
...
- **Anchors are named, not linked.** Name the entities, seams, screens and reuse targets **by name**; the read-first list carries the paths. Naming them carries one vocabulary from here into the glossary and into the spec, and tells the agent which part of a long file matters. Never copy a file's content into the block.
...
Paste the briefing block, resolve the open questions, and let the session sharpen the glossary and raise an ADR if it earns one.

```
/impeccable shape <feature>          UI only
```

Plan the UX before any code. Its output feeds the spec — it does not replace it.

```
/to-spec
```

Synthesises the conversation into a spec on the tracker. It will propose the seams — check them against the Anchors before confirming.

```
/to-tickets
```

Breaks the spec into tracer-bullet tickets with blocking edges. Approve the granularity before it publishes. If it proposes far more tickets than the neighbouring lines needed, this roadmap line was mis-cut — go back and split it rather than pushing on.

```
/implement
```

One ticket at a time, working the **frontier**. Each ticket is sized to a fresh context window, so clear context between them. `/implement` drives `/tdd` at the agreed seams and calls `/code-review` before committing; on a UI ticket the Impeccable detector hook also fires on every edit.

```
/impeccable critique <feature>       UI only
/impeccable audit <feature>          UI only
/impeccable polish <feature>         UI only
```

After the last ticket, before merge. `critique` reviews hierarchy and clarity, `audit` covers accessibility, performance and responsiveness, `polish` aligns to the design system and checks shipping readiness. These are design passes — `/code-review` already covered standards and spec compliance per ticket.
